...
export default abstract class HackerNews {
   ...
   */
  public static getUpdates(): Promise<{
  ...
  public static getStories(type: HackerNewsStoryType, offset: number = 0, amount: number): Promise<HackerNewsItem[]> {
    return this.getStoryIds(type).then((storyIds: number[]) => {
      if (amount)
        storyIds = storyIds.slice(
          Math.min(offset, storyIds.length - 1),
          Math.min(offset + amount, storyIds.length - 1)
        );
